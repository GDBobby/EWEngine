...
{
    //IM_ASSERT((start_type == ImNodesAttributeType_Input) || (start_type == ImNodesAttributeType_Output));
    ...
    const ImVec2               offset)
{
    ...
    ImVec2 center = node_rect.GetTL();
    center.x += offset.x * node_rect.GetWidth();
    center.y += offset.y * node_rect.GetHeight();
    return center;
}
...
ImVec2 GetScreenSpacePinCoordinates(const ImNodesEditorContext& editor, const ImPinData& pin)
{
    ...
    return GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect, pin.offset);
}
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx)
    {
        if (editor.Links.InUse[link_idx])
        {
            ...
            const ImVec2 start = GetScreenSpacePinCoordinates(
                node_start_rect, pin_start.AttributeRect, pin_start.offset);
            const ImVec2 end =
                GetScreenSpacePinCoordinates(node_end_rect, pin_end.AttributeRect, pin_end.offset);
    ...
    const ImOptionalIndex       duplicate_link)
{
    ...
    if ((start_pin.Type == end_pin.Type) && (start_pin.Type != ImNodesAttributeType_None && start_pin.Type != ImNodesAttributeType_None))
    {
        return false;
    }
    ...
    const ImVector<int>&           occluded_pin_indices)
{
    ...
    for (int idx = 0; idx < pins.Pool.Size; ++idx)
    {
        ...
        if (distance_sqr < hover_radius_sqr && distance_sqr < smallest_distance)
        {
            ...
            pins.Pool[idx].hovered = true;
            if (lastPin >= 0) {
                pins.Pool[lastPin].hovered = false;
                lastPin = idx;
            }
        }
    }
...
void DrawPin(ImNodesEditorContext& editor, const int pin_idx)
{
    ...
    pin.Pos = GetScreenSpacePinCoordinates(parent_node_rect, pin.AttributeRect, pin.offset);
...
int EndNodeEditor()
{
        ...
        MouseInCanvas() && !IsMiniMapHovered())
    {
        ...
        if (!GImNodes->HoveredNodeIdx.HasValue())
        {
            ...
            ret = editor.Nodes.Pool[GImNodes->HoveredNodeIdx.Value()].Id;
        }
...
bool BeginPinAttribute(
    const int                  id,
    const ImVec2               pinOffset,
    const ImNodesAttributeType type,
    const ImNodesPinShape      shape)
{
    // Make sure to call BeginNode() before calling
    ...
    ImPinData& pin = editor.Pins.Pool[pin_idx];
    pin.Id = id;
    pin.ParentNodeIdx = GImNodes->CurrentNodeIdx;
    pin.Type = type;
    pin.Shape = shape;
    pin.Flags = GImNodes->CurrentAttributeFlags;
...
void EndInputAttribute() { ImNodes::EndPinAttribute(); }
...
void EndOutputAttribute() { ImNodes::EndPinAttribute(); }
...
bool CheckPinLinking(int pinIndex) {
    ...
    if (is_started)
    {
        const ImNodesEditorContext& editor = EditorContextGet();
        const int                   pin_idx = editor.ClickInteraction.LinkCreation.StartPinIdx;
        return pinIndex == editor.Pins.Pool[pin_idx].Id;
    }
