#include "EWEngine/Graphics/Camera.h"

namespace EWE {
	Camera::Camera() 
		: buffers{
			*Global::logicalDevice,
			sizeof(CameraBufferObject), 1,
			VmaAllocationCreateInfo{
				.flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT,
				.usage = VMA_MEMORY_USAGE_AUTO
			}
		}
	{
		data.view.At(3, 3) = 1.f;
	}


	void Camera::SetBuffers() {

	}

	void Camera::SetOrthographicProjection(float left, float right, float top, float bottom, float near, float far) {
		//projection = lab::mat4{ 1.0f };
		data.proj = lab::OrthographicMatrix(bottom, top, left, right, near, far);
	}

	void Camera::SetPerspectiveProjection(float fovy, float aspect, float near, float far) {
		//inverting aspect, to make it height / width instead of width / height
		data.proj = lab::ProjectionMatrix<lab::Perspective::Vulkan>(fovy, aspect, near, far);
		//projection = lab::CreateProjectionMatrix(fovy * 1.1f, aspect * 1.1f, near * 1.1f, far * 1.1f);
		//projection = glm::perspective(fovy, aspect, near, far);
		conservativeProjection = lab::ProjectionMatrix<lab::Perspective::Vulkan>(fovy * 1.1f, aspect * 1.1f, near * 1.1f, far * 1.1f);
	}


	void Camera::UpdateBothBuffers() {
		for (uint8_t i = 0; i < max_frames_in_flight; i++) {
			UpdateBuffer(i);
		}
	}
	void Camera::UpdateBuffer(uint8_t frameIndex) {
		memcpy(buffers[frameIndex].mapped, &data, sizeof(CameraBufferObject));
		buffers[frameIndex].Flush();
	}
	void Camera::UpdateBuffer() {
		UpdateBuffer(Global::frameIndex);
	}

	void Camera::UpdateViewData(lab::vec3 const& position, lab::vec3 const& target, lab::vec3 const& cameraUp) {
		//probably store a position, target, and camera up variable in this class, then hand out a pointer to those variables
		//being lazy rn
		this->target = target;
		this->cameraUp = cameraUp;
		data.cameraPos = position;
		dataHasBeenUpdated = max_frames_in_flight;
	}
	
	constexpr std::array<lab::vec4, 6> GetFrustumPlanes_Impl(lab::mat4 const& projView) {

		enum side { LEFT = 0, RIGHT = 1, TOP = 2, BOTTOM = 3, NEAR = 4, FAR = 5 };
		std::array<lab::vec4, 6> planes;
		planes[LEFT].x = projView.columns[0].w + projView.columns[0].x;
		planes[LEFT].y = projView.columns[1].w + projView.columns[1].x;
		planes[LEFT].z = projView.columns[2].w + projView.columns[2].x;
		planes[LEFT].w = projView.columns[3].w + projView.columns[3].x;

		planes[RIGHT].x = projView.columns[0].w - projView.columns[0].x;
		planes[RIGHT].y = projView.columns[1].w - projView.columns[1].x;
		planes[RIGHT].z = projView.columns[2].w - projView.columns[2].x;
		planes[RIGHT].w = projView.columns[3].w - projView.columns[3].x;

		planes[TOP].x = projView.columns[0].w - projView.columns[0].y;
		planes[TOP].y = projView.columns[1].w - projView.columns[1].y;
		planes[TOP].z = projView.columns[2].w - projView.columns[2].y;
		planes[TOP].w = projView.columns[3].w - projView.columns[3].y;

		planes[BOTTOM].x = projView.columns[0].w + projView.columns[0].y;
		planes[BOTTOM].y = projView.columns[1].w + projView.columns[1].y;
		planes[BOTTOM].z = projView.columns[2].w + projView.columns[2].y;
		planes[BOTTOM].w = projView.columns[3].w + projView.columns[3].y;

		planes[NEAR].x = projView.columns[0].w + projView.columns[0].z;
		planes[NEAR].y = projView.columns[1].w + projView.columns[1].z;
		planes[NEAR].z = projView.columns[2].w + projView.columns[2].z;
		planes[NEAR].w = projView.columns[3].w + projView.columns[3].z;

		planes[FAR].x = projView.columns[0].w - projView.columns[0].z;
		planes[FAR].y = projView.columns[1].w - projView.columns[1].z;
		planes[FAR].z = projView.columns[2].w - projView.columns[2].z;
		planes[FAR].w = projView.columns[3].w - projView.columns[3].z;

		return planes;
	}

	std::array<lab::vec4, 6> Camera::GetFrustumPlanes(){
		return GetFrustumPlanes_Impl(data.projView);
	}
		
	std::array<lab::vec4, 6> Camera::GetConservativeFrustumPlanes(const lab::vec3 position, const lab::vec3 rotation) {

		const float c3 = lab::Cos(rotation.z);
		const float s3 = lab::Sin(rotation.z);
		const float c2 = lab::Cos(rotation.x);
		const float s2 = lab::Sin(rotation.x);
		const float c1 = lab::Cos(rotation.y);
		const float s1 = lab::Sin(rotation.y);
		const lab::vec3 forwardDir = lab::Normalized(lab::vec3{ s1, -s2, c1 });
		const lab::vec3 conversativePosition = position + forwardDir * 5.f;
		const lab::vec3 u{ (c1 * c3 + s1 * s2 * s3), (c2 * s3), (c1 * s2 * s3 - c3 * s1)};
		const lab::vec3 v{ (c3 * s1 * s2 - c1 * s3), (c2 * c3), (c1 * c3 * s2 + s1 * s3)};
		const lab::vec3 w{ (c2 * s1), (-s2), (c1 * c2)};
		//view = lab::mat4{ 1.f };

		lab::mat4 conservativeView;

		conservativeView.columns[0][3] = 0.f;
		conservativeView.columns[1][3] = 0.f;
		conservativeView.columns[2][3] = 0.f;

		conservativeView.columns[0][0] = u.x;
		conservativeView.columns[1][0] = u.y;
		conservativeView.columns[2][0] = u.z;
		conservativeView.columns[0][1] = v.x;
		conservativeView.columns[1][1] = v.y;
		conservativeView.columns[2][1] = v.z;
		conservativeView.columns[0][2] = w.x;
		conservativeView.columns[1][2] = w.y;
		conservativeView.columns[2][2] = w.z;
		conservativeView.columns[3][0] = -lab::Dot(u, conversativePosition);
		conservativeView.columns[3][1] = -lab::Dot(v, conversativePosition);
		conservativeView.columns[3][2] = -lab::Dot(w, conversativePosition);
		conservativeView.columns[3][3] = 1.f;

		const lab::mat4 conservativeProjView = conservativeProjection * conservativeView;

		return GetFrustumPlanes_Impl(conservativeProjView);
	}
}